...
void run_RealRW(benchmark::State &state, std::size_t n, T t)
{
    ...
    for (auto _ : state) {
        ...
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x0, i) += 2.f; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x1, i) += 2.f; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x2, i) += 2.0; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x3, i) += 2.0; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x4, i) += 2; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x5, i) += 2; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x6, i) += v2; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x7, i) += v2; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x8, i) += m2; });
        parallel_for_n<B>(n, [=] BACKEND_HOST_DEVICE (std::size_t i) mutable { MEMBER_ACCESS(t, x9, i) += m2; });
    }
...
BENCHMARK_TEMPLATE_METHOD_F(Fixture1, RealRW)(benchmark::State &state)
{
    ...
    run_RealRW<B>(state, this->n, this->t);
}
