...
struct is_aggregate_constructible_from_n_impl<T, std::index_sequence<Is...>,
                                              std::void_t<decltype(T{(void(Is), UniversalType{})...})>>
    : std::true_type {};
...
struct is_aggregate_constructible_from_n {
    constexpr static bool value = is_aggregate_constructible_from_n_helper<T, N>::value &&
                                  !is_aggregate_constructible_from_n_helper<T, N + 1>::value;
};
...
{
    if constexpr (detail::is_aggregate_constructible_from_n<Argument, 2>::value)
        return 2;
    else if constexpr (detail::is_aggregate_constructible_from_n<Argument, 3>::value)
        return 3;
    else if constexpr (detail::is_aggregate_constructible_from_n<Argument, 4>::value)
        return 4;
    else if constexpr (detail::is_aggregate_constructible_from_n<Argument, 6>::value)
        return 6;
    else if constexpr (detail::is_aggregate_constructible_from_n<Argument, 10>::value)
        return 10;
    else if constexpr (detail::is_aggregate_constructible_from_n<Argument, 32>::value)
        return 32;
    else if constexpr (detail::is_aggregate_constructible_from_n<Argument, 64>::value)
        return 64;
    else
        return 100; // Silence warnings about missing return value
}

template <class Argument, class FunctionObject>
[[gnu::always_inline]] constexpr auto invoke(Argument &arg, FunctionObject &&f)
{
    ...
    if constexpr (M == 2) {
        ...
        auto &[m00, m01, m02, m03, m04, m05, m06, m07, m08, m09, m10, m11, m12, m13, m14, m15, m16, m17, m18, m19, m20,
               m21, m22, m23, m24, m25, m26, m27, m28, m29, m30, m31] = arg;
        return f(m00, m01, m02, m03, m04, m05, m06, m07, m08, m09, m10, m11, m12, m13, m14, m15, m16, m17, m18, m19,
        ...
        auto &[m00, m01, m02, m03, m04, m05, m06, m07, m08, m09, m10, m11, m12, m13, m14, m15, m16, m17, m18, m19, m20,
               ...
               m42, m43, m44, m45, m46, m47, m48, m49, m50, m51, m52, m53, m54, m55, m56, m57, m58, m59, m60, m61, m62,
               m63] = arg;
        return f(m00, m01, m02, m03, m04, m05, m06, m07, m08, m09, m10, m11, m12, m13, m14, m15, m16, m17, m18, m19,
...
template <template <class> class F_out, template <class> class F_in, template <template <class> class> class S,
          class FunctionObject>
struct memberwise {
    FunctionObject f;

    template <class... Args>
    [[gnu::always_inline]] constexpr S<F_out> operator()(Args &...args) const
    {
        return { f.template operator()<F_in>(args)... };
    }
};
