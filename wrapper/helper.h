...
constexpr std::size_t CountMembers() {
    ...
    else if constexpr (detail::is_aggregate_constructible_from_n<Argument, 32>::value) return 32;
...
template <
    ...
    class  FunctionObject
>
[[gnu::always_inline]] constexpr S<F_out> invoke_on_members(const S<F_in> & s, FunctionObject&& f) {
    return invoke(s, memberwise<F_out, F_in, S, FunctionObject>{f});
}
