...
def plot_nmembers_heatmaps(df, app, scaled=False):
    ...
    for il, layout in enumerate(["aos", "soa"]):
        ...
        for i, k in enumerate(df.columns[3:]):
            ...
            ims.append(plt.imshow(pivot, cmap=v["colormap"], aspect="auto", origin="lower"))
        ...
        for i, (k, v) in enumerate(zip(derived_values.keys(), derived_values.values())):
            ...
            plt.yticks(np.arange(len(pivot.index)), pivot.index)
            if v["ymax"]:
                ymax[il][i + n_values] = np.max(pivot.values)
            plt.tight_layout()

        imgs.append(ims)

    if scaled:
        for ic, ims_l in enumerate(imgs):
            for ia, im in enumerate(ims_l):
                max_val = np.max(ymax[:, ia])
                im.set_clim(0, max_val) if max_val > 0 else im.set_clim(vmin=0)
