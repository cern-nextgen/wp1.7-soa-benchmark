#define AOS_MANUAL

#include <Eigen/Core>
#include "benchmarks/common.h"

struct S2 {
    int x0, x1;
};

struct S10 {
    float x0, x1;
    double x2, x3;
    int x4, x5;
    Eigen::Vector3d x6, x7;
    Eigen::Matrix3d x8, x9;
};

struct S32 {
    uint32_t x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15,
          x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27, x28, x29, x30, x31;
};

struct S64 {
    float x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12;
    double x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
    int x26, x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38;
    Eigen::Vector3d x39, x40, x41, x42, x43, x44, x45, x46, x47, x48, x49, x50;
    Eigen::Matrix3d x51, x52, x53, x54, x55, x56, x57, x58, x59, x60, x61, x62, x63;
};

struct Snbody {
    float x, y, z, vx, vy, vz;
};

struct Sstencil {
    double src, dst, rhs;
};

// Realistic AoS particle with extra fields beyond (x,y,z,M) to model real-world padding cost
struct Particle {
    int id;
    double x, y, z, M;
    double fX, fY, fZ, fM;
    double poscovmatrix[9];
};

/// Fixtures ///

template <typename S, typename N, Backend B = Backend::CPU>
class Fixture1 : public benchmark::Fixture {
public:
    static constexpr auto n = N::value;
    static constexpr Backend backend = B;
    S* t = nullptr;

    void SetUp(benchmark::State&) override   { t = backend_allocator<B>::template alloc<S>(n); }
    void TearDown(benchmark::State&) override { backend_allocator<B>::free(t); t = nullptr; }
};

template <typename S1, typename S2, typename N, Backend B = Backend::CPU>
class Fixture2 : public benchmark::Fixture {
public:
    static constexpr auto n = N::value;
    static constexpr Backend backend = B;
    S1* t1 = nullptr;
    S2* t2 = nullptr;

    void SetUp(benchmark::State&) override {
        t1 = backend_allocator<B>::template alloc<S1>(n);
        t2 = backend_allocator<B>::template alloc<S2>(n);
    }
    void TearDown(benchmark::State&) override {
        backend_allocator<B>::free(t1); t1 = nullptr;
        backend_allocator<B>::free(t2); t2 = nullptr;
    }
};

/// Benchmarks ///

#include "benchmarks/easy.h"
#include "benchmarks/easycompute.h"
#include "benchmarks/real.h"
#include "benchmarks/strided.h"
#include "benchmarks/hard.h"
#include "benchmarks/nbody.h"
#include "benchmarks/stencil.h"
#include "benchmarks/invmass.h"

INSTANTIATE_BENCHMARKS_F1(EasyRW,      S2,       N_Large);
INSTANTIATE_BENCHMARKS_F1(EasyCompute, S2,       N);
INSTANTIATE_BENCHMARKS_F1(RealRW,      S10,      N);
INSTANTIATE_BENCHMARKS_F1(Strided,     S32,      N_Large);
INSTANTIATE_BENCHMARKS_F1(HardRW,      S64,      N);
INSTANTIATE_BENCHMARKS_F1(NBody,       Snbody,   N);
INSTANTIATE_BENCHMARKS_F1(Stencil,     Sstencil, N_Large);

INSTANTIATE_BENCHMARKS_F2(InvariantMass, Particle, Particle, N_Large);

int main(int argc, char** argv) {
    ::benchmark::Initialize(&argc, argv);
    ::benchmark::AddCustomContext("name", "Manual AoS CPU");
    if (::benchmark::ReportUnrecognizedArguments(argc, argv)) return 1;
    ::benchmark::RunSpecifiedBenchmarks();
    ::benchmark::Shutdown();
    return 0;
}
